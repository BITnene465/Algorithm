...
struct EK{
    ...
    vector<int> G[MAXN];  // G[x][i]表示节点x发出的第i条边的id，访问edges[id]即可
    ll a[MAXN],p[MAXN];
...
int main(void)
{
    ...
    // 建立超级源点和超级汇点
    for(int i=1;i<=m;++i)mf.adde(n+1,i,1);
    for(int i=m+1;i<=n;++i)mf.adde(i,n+2,1);
    cout<<mf.Maxflow(n+1,n+2)<<endl;
    mf.search(m);
    return 0;
}